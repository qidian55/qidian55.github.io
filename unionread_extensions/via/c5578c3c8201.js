;$('#addonModal').on('show.bs.modal',function(event){var button=$(event.relatedTarget)
...
function operate(id){var i=findAddon(id)
var addon_json=JSON.stringify({'id':i.data('id'),'name':i.data('name'),'author':i.data('author'),'url':i.data('url'),'code':i.data('code')})
var base64=new Base64()
var addon_result=base64.encode(addon_json)
try{window.via.addon(addon_result)}catch(error){console.log(error)}
setStauts(id,i.data('status')==0?1:0)}
function setStauts(id,status){findAddon(id).data('status',status)
$('#operate-'+id).html(status==0?lang['install']:lang['uninstall'])}
function findAddon(id){return $('#addon-'+id)}
function submitLang(){$('#lang-form').submit()}
$(document).ready(function(){var data='[]'
try{data=window.via.getInstalledAddonID()}catch(error){console.log(error)}
var json=JSON.parse(data)
for(var i=0;i<json.length;i++){setStauts(json[i],1)}})
